...
void cl_altaClienteForzada(Cliente* pCliente,int limite,char* nombre,char* apellido, int cuit)
{
    int i;

    if(limite > 0 && pCliente != NULL)
    {
        i = cl_getEspacioVacio(pCliente,limite);
        if(i >= 0)
        {
            strcpy(pCliente[i].nombre,nombre);
            strcpy(pCliente[i].apellido,apellido);
            pCliente[i].cuit = cuit;
            pCliente[i].id = generarId();
            pCliente[i].isEmpty = 0;
        }
    }
}
