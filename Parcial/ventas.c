                            ...
                            int idCliente, int zona, int cuitCliente, int estado)
{
    ...
    if(limite > 0 && pVentas != NULL)
    {
        i = ven_getEspacioVacio(pVentas,limite);
        if(i >= 0)
        {
            strcpy(pVentas[i].nombreArchivo,nombreArchivo);
            pVentas[i].cantidadAfiches = cantidadDeAfiches;
            pVentas[i].cuitCliente = cuitCliente;
            pVentas[i].id = generarId();
            pVentas[i].isEmpty = 0;
            pVentas[i].idCliente = idCliente;
            pVentas[i].zona = zona;
            pVentas[i].estado = estado;
        }
    }
}
...
int ven_cantidadVentas_aCobrar(Ventas* pVentas, int limite,int id)
{
    ...
    for(i=0;i<limite;i++)
    {
        if(pVentas[i].idCliente== id && pVentas[i].estado==1)
        {
            aux= aux+1;
        }
    }
...
int ven_cantidadVentas_cobradas(Ventas* pVentas, int limite, int id)
{
    ...
    for(i=0;i<limite;i++)
    {
        if(pVentas[i].idCliente == id && pVentas[i].estado==3)
        {
            aux= aux+1;
        }
    }
...
int ven_cantidadAfiches_aCobrar(Ventas* pVentas, int limite, int id)
{
    int i;
    int max = 0;
    Informe auxInforme[limite];

    for(i=0;i<limite;i++)
    {
        if(pVentas[i].idCliente == id && pVentas[i].estado==1)
        {
            auxInforme[i].afichesACobrar = pVentas[i].cantidadAfiches;
        }
    }

    for(i=0;i<limite;i++)
    {
        if(auxInforme[i].afichesACobrar > max)
        {
            max = i;
        }
